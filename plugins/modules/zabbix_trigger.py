...
class Trigger(ZabbixBase):
    ...
    def check_trigger_changed(self, old_trigger, keys):
        try:
            ...
            self._module.fail_json(msg="Failed to get trigger: %s" % e)
        # ponytail: compare only the fields we manage. Zabbix auto-populates some unmanaged fields
        # (e.g. event_name/url_name) during trigger.update, which flipped the old!=new full-dict compare
        # and broke idempotence. Both dicts come from trigger.get, so no int/str or expression-normalize skew.
