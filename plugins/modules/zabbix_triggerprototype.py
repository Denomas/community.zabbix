...
class Triggerprototype(ZabbixBase):
        ...
        try:
            if templated:
                triggerprototypes = self._zapi.triggerprototype.get(
                    {'filter': {'description': triggerprototype_name, 'host': host,
                                'templated': templated}})
            else:
                triggerprototypes = self._zapi.triggerprototype.get(
                    {'filter': {'description': triggerprototype_name, 'host': host,
                                'templated': templated, 'templateid': 0}})
    ...
    def check_triggerprototype_changed(self, old_triggerprototype, keys):
        try:
            ...
            self._module.fail_json(msg="Failed to get triggerprototype: %s" % e)
        # ponytail: compare only managed fields (see zabbix_trigger) so unmanaged fields Zabbix
        # auto-populates during update don't break idempotence.
